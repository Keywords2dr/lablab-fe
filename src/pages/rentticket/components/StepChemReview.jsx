...
import {
  ...
  SchoolOutlined, BadgeOutlined, CheckCircleOutlined, ScienceOutlined
} from "@mui/icons-material";

export default function StepReview({ selectedRoom, selectedChemicals = {}, form }) {
  const fmt = (dateStr) => {
    if (!dateStr) return "—";
  ...
  const isChemTicket = chemicals.length > 0 || form.ticketType === "CHEMICAL_ONLY";
  ...
  return (
    <div className="rr-card">
      ...
      <div className="rr-card-body">
        <div className="rr-summary-grid">
          ...
          <div className="rr-summary-item span-2">
            ...
            <div className="rr-summary-value" style={{ whiteSpace: "pre-wrap" }}>
              {form.purposeType === "TEACHING" ? "Giảng dạy" : 
               form.purposeType === "RESEARCH" ? "Nghiên cứu" : 
               form.purposeType === "PRACTICE" ? "Thực hành" : 
               form.purposeType || "—"}
            </div>
          </div>
        ...
        {isChemTicket && (
          <div className="cr-chem-section" style={{ marginTop: 24, borderTop: "1px solid #e2e8f0", paddingTop: 20 }}>
            <div className="cr-chem-section-title" style={{ display: "flex", alignItems: "center", gap: 8, fontWeight: 600, marginBottom: 16 }}>
              <ScienceOutlined style={{ fontSize: 20, color: "#0f766e" }} />
              Danh sách hóa chất mượn ({chemicals.length} loại)
            </div>

            {chemicals.length === 0 ? (
              <div style={{ color: "#94a3b8", fontSize: "0.875rem", padding: "12px 0" }}>
                Chưa có hóa chất nào được chọn.
              </div>
            ) : (
              <div className="cr-chem-table-wrap" style={{ overflowX: "auto" }}>
                <table className="cr-chem-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.9rem" }}>
                  <thead>
                  ...
                  <tbody>
                    {chemicals.map(({ chemical, quantity }, idx) => (
                      <tr key={chemical.id} style={{ borderBottom: "1px solid #e2e8f0" }}>
                        <td style={{ padding: "10px 12px" }}>{idx + 1}</td>
                        <td style={{ padding: "10px 12px" }}>{chemical.name}</td>
                        <td style={{ padding: "10px 12px" }}>{chemical.formula || "—"}</td>
                        ...
                        <td style={{ padding: "10px 12px" }}><strong>{quantity}</strong></td>
                        <td style={{ padding: "10px 12px" }}>{chemical.unit}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
