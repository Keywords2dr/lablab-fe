...
function mapInventoryToChemical(inv) {
  ...
  const total     = parseFloat(inv.totalQuantity  ?? inv.total     ?? item.totalQuantity  ?? 0);
  const locked    = parseFloat(inv.lockedQuantity ?? inv.locked    ?? item.lockedQuantity ?? 0);
  ...
  return {
    ...
    hazardLevel: (item.hazardLevel ?? item.hazard ?? item.hazardLevelName ?? "LOW").toUpperCase(),
    available,
  };
}
...
export default function StepRoomAndChemicals({
  ...
  useEffect(() => {
    ...
    const fetchChemicals = async () => {
      ...
      try {
        ...
        setChemState({ chemicals: mapped, loadingChem: false, chemError: null });
      } catch (err) {
        if (ctrl.signal.aborted) return;
  ...
  const filteredChems = chemicals.filter((c) => {
    ...
    return name.includes(chemSearch.toLowerCase()) || code.includes(chemSearch.toLowerCase());
  });
  ...
  const handleQtyChange = (chemId, delta) => {
    setSelectedChemicals((prev) => {
      ...
      let currentQty = parseFloat(prev[chemId].quantity) || 0;
      ...
      <div className="rac-split">
        ...
        <div className="rac-left">
          ...
          <div className="rac-search-box">
            ...
            />
            {roomSearch && (
              <button className="rac-search-clear" onClick={() => setRoomSearch("")}>✕</button>
            )}
          </div>

          {loading ? (
            <div className="rac-loading">Đang tải phòng...</div>
          ) : rooms.length === 0 ? (
                  ...
                  >
                    <div className="rac-room-card-left">
                      ...
                      <div className="rac-room-info">
                        <div className="rac-room-name">{room.name ?? room.roomName}</div>
                        {room.location && (
                        ...
                        {room.managerName && (
                          <div className="rac-room-manager">
                            <SupervisorAccountOutlined style={{ fontSize: 12 }} />
                            {room.managerName}
                          </div>
                        )}
                        {isInactive && (
                          <span className="rac-room-inactive-badge">Không hoạt động</span>
                        )}
                      </div>
                    </div>
                    <div className="rac-room-card-right">
                      {isSelected
                        ? <CheckCircleOutlined style={{ fontSize: 18, color: "#0284c7" }} />
                        : !isInactive && <KeyboardArrowRightOutlined style={{ fontSize: 18, color: "#94a3b8" }} />}
                    </div>
                  </div>
                );
              })}
          ...
          {totalPages > 1 && (
            <div style={{
              ...
              gap: 8, padding: "10px 0 4px", marginTop: 6,
          ...
          {!selectedRoom ? (
            ...
            <>
              <div className="rac-panel-header">
                ...
                {totalSelected > 0 && (
                  <span className="rac-badge-selected">{totalSelected} đã chọn</span>
                )}
              </div>
              ...
              <div className="rac-chem-layout">
                ...
                <div className="rac-chem-col">
                  <div className="rac-search-box" style={{ marginBottom: 10 }}>
                    ...
                    />
                    {chemSearch && (
                      <button className="rac-search-clear" onClick={() => setChemSearch("")}>✕</button>
                    )}
                  </div>
                  ...
                  {loadingChem ? (
                    ...
                    <div className="rac-chem-list">
                      {filteredChems.map((chem) => {
                        ...
                        const hazard     = HAZARD_CONFIG[chem.hazardLevel] ?? HAZARD_CONFIG.LOW;
                        ...
                        return (
                          ...
                          >
                            <div className={`rac-chem-check${isSel ? " rac-chem-check--on" : ""}`}>
                              {isSel && <CheckCircleOutlined style={{ fontSize: 13 }} />}
                            </div>
                            ...
                            <div className="rac-chem-info">
                              ...
                              <div className="rac-chem-meta-row">
                                {chem.itemCode && (
                                  <span className="rac-cas">Mã HC: {chem.itemCode}</span>
                                )}
                                {chem.formula && (
                                  <span className="rac-cas" style={{ marginLeft: 8 }}>
                                ...
                                >
                                  {chem.hazardLevel === "HIGH" && (
                                    <WarningAmberOutlined style={{ fontSize: 10 }} />
                                  )}
                                  {hazard.label}
                                </span>
                              </div>
                              <div className="rac-chem-avail">
                                Khả dụng:{" "}
                                <strong style={{ color: isLowStock ? "#ef4444" : "#0f172a" }}>
                                  {chem.available} {chem.unit}
                                </strong>
                                {isLowStock && (
                                  <span className="rac-low-stock"> ⚠ Sắp hết</span>
                                )}
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="rac-cart-item-bottom">
                            <div className="rac-qty-ctrl">
                              ...
                              <input
                                ...
                                value={quantity}
                                onChange={(e) => handleQtyInput(chemical.id, e.target.value)}
                              ...
                              <button
                                ...
                                disabled={parseFloat(quantity) >= chemical.available}
                              >
