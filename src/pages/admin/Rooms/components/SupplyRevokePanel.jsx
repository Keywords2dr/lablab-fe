...
function ItemCardRevoke({
  ...
  return (
    ...
    >
      ...
      <div
        className="stp-item-card__meta"
        style={{ display: "flex", flexDirection: "column", gap: "4px" }}
      >
        {item.formula && (
          <span
            className="stp-item-card__cat"
        ...
        >
          <span>
            Tổng: {item.totalQuantity !== undefined ? item.totalQuantity : "?"}
          </span>
          <span>
            Khóa:{" "}
            {item.lockedQuantity !== undefined ? item.lockedQuantity : "?"}
          </span>
          <span style={{ color: "#059669", fontWeight: 600 }}>
            Khả dụng: {maxQuantity} {item.unit || "gói"}
          </span>
        </div>
        {item.amountPerPackage && (
          <div style={{ fontSize: "11px", color: "#8b5cf6", fontWeight: 500 }}>
            Quy cách: {item.amountPerPackage} {item.unit} /{" "}
            {item.packaging || "gói"}
          </div>
        )}
      </div>
...
function RoomItemSectionRevoke({
  ...
  Object.entries(roomItems).forEach(([itemId, cnt]) => {
    ...
    if (!err) {
      const itemInv = items.find(
        (i) => i.itemId === itemId || i.itemCode === itemId,
      );
      const itemGlobal = globalItems.find(
        (i) => i.itemId === itemId || i.itemCode === itemId,
      );
      if (itemInv) {
        const maxPkgs = getMaxPackages(
          itemInv.availableQuantity,
          itemGlobal?.amountPerPackage,
        );
        if (cnt > maxPkgs) {
          err = "Vượt tồn";
        }
  ...
  return (
    <div className="stp-room-section">
      <div
        className={`stp-room-section__header ${hasError && selectedCount > 0 ? "stp-room-section__header--warn" : ""}`}
      >
        <button
                ...
                return (
                  <ItemCardRevoke
                    ...
                    onChangeCount={(id, val) =>
                      onChangeCount(room.roomId, uid, val)
                    }
...
export default function SupplyRevokePanel({ rooms = [] }) {
  ...
  if (subMode === "byRoom") {
      ...
      .map((roomId) => {
        ...
        const itemsList = Object.entries(itemsMap).map(([itemId, count]) => {
          ...
          return {
            ...
            count,
            error,
            itemName: itemInv?.itemName || "Unknown",
            packaging: itemGlobal?.packaging || "gói",
          };
        });
    ...
    canConfirm =
      ...
      summaryData.every((d) => d.items.every((i) => !i.error)) &&
      !submitting;
  } else {
    ...
    Object.entries(itemRoomsData).forEach(([itemId, roomsMap]) => {
      ...
      Object.entries(roomsMap).forEach(([roomId, count]) => {
        ...
        roomItemsMap[roomId].items.push({
          ...
          count,
          error,
          itemName: itemGlobal?.itemName || "Unknown",
          packaging: itemGlobal?.packaging || "gói",
        });
      });
    });
    ...
    <div className="stp-root">
      ...
      <div className="stp-body stp-body--3col">
          ...
          <div className="stp-card stp-card--full-height">
            <div className="stp-card__header">
              <StepDot
                ...
                status={
                  subMode === "byRoom"
                    ? selectedRoomsForByRoom.length > 0
                      ? "done"
                      : "active"
                    : selectedItemForByItem
                      ? "done"
            ...
            <div className="stp-search-row">
              ...
              <input
                className="stp-input"
                placeholder={
                  subMode === "byRoom" ? "Tìm phòng..." : "Tìm hóa chất..."
                }
                value={search1}
              ...
              {subMode === "byRoom" ? (
                rooms
                  .filter(
                    (r) =>
                      !search1 ||
                      r.roomName.toLowerCase().includes(search1.toLowerCase()),
                  )
                  .map((room) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => toggleRoomInByRoom(room.roomId)}
                      >
                        ...
                        {sel && (
                          <div
                            className="stp-item-card__check"
                            style={{
                              position: "static",
                              margin: 0,
                              padding: 0,
                ...
                itemsInRooms
                  .filter(
                    (i) =>
                      ...
                      i.itemCode?.toLowerCase().includes(search1.toLowerCase()),
                  )
                  .map((item) => {
                    const uid = item.itemId || item.itemCode;
                    const isActive = selectedItemForByItem === uid;
                    const selectedRoomsCount = Object.keys(
                      itemRoomsData[uid] || {},
                    ).length;
                    return (
                      <button
                        key={uid}
                        className={`stp-room-item ${isActive ? "stp-room-item--selected" : ""}`}
                        onClick={() => setSelectedItemForByItem(uid)}
                      >
        ...
        <div className="stp-col" style={{ gridColumn: "2" }}>
          <div
            className={`stp-card ${subMode === "byRoom" ? (selectedRoomsForByRoom.length === 0 ? "stp-card--dim" : "") : !selectedItemForByItem ? "stp-card--dim" : ""}`}
          >
            <div className="stp-card__header">
              <StepDot
                num={2}
                status={
                  (
                    subMode === "byRoom"
                      ? summaryData.length > 0
                      : Object.keys(itemRoomsData).length > 0
                  )
                    ? "done"
                    : "active"
                }
            ...
            >
              {subMode === "byRoom"
                ? selectedRoomsForByRoom.map((roomId) => {
                    ...
                    const filteredInv = roomInv.filter(
                      (i) =>
                        (!search2 ||
                          i.itemName
                            ?.toLowerCase()
                ...
                : selectedItemForByItem &&
                  (() => {
                    const roomsWithItem = rooms.filter((r) => {
                      ...
                      return inv.some(
                        (i) =>
                          (i.itemId === selectedItemForByItem ||
                            i.itemCode === selectedItemForByItem) &&
                          i.availableQuantity > 0,
                      );
                    });

                    const filteredRooms = roomsWithItem.filter(
                      (r) =>
                        !search2 ||
                        r.roomName
                          .toLowerCase()
                    ...
                    const globalItem = globalItems.find(
                      (g) =>
                        g.itemId === selectedItemForByItem ||
                        g.itemCode === selectedItemForByItem,
                    );

                    if (filteredRooms.length === 0)
                      return (
                        <div style={{ padding: 20 }}>
                          Không có phòng nào chứa hóa chất này.
                        </div>
                      );
                    ...
                    return (
                      <div className="stp-item-grid stp-item-grid--compact">
                        {filteredRooms.map((room) => {
                          const invItem = (inventories[room.roomId] || []).find(
                            (i) =>
                              i.itemId === selectedItemForByItem ||
                              i.itemCode === selectedItemForByItem,
                          );
                          const sel =
                            (itemRoomsData[selectedItemForByItem] || {})[
                              room.roomId
                            ] !== undefined;
                          const packageCount = (itemRoomsData[
                            selectedItemForByItem
                          ] || {})[room.roomId];
                          const maxPkgs = getMaxPackages(
                          ...
                          let error = sel
                            ? validatePackageCount(packageCount)
                            : false;
                          if (!error && sel) {
                          ...
                          return (
                            <ItemCardBase
                              ...
                              maxPackages={maxPkgs}
                            >
                              <div className="stp-item-card__name">
                                {room.roomName}
                              </div>
                              ...
                              >
                                ...
                                >
                                  <span>
                                    Tổng:{" "}
                                    {invItem.totalQuantity !== undefined
                                      ? invItem.totalQuantity
                                      : "?"}
                                  </span>
                                  <span>
                                    Khóa:{" "}
                                    {invItem.lockedQuantity !== undefined
                                      ? invItem.lockedQuantity
                                      : "?"}
                                  </span>
                                  <span
                                    style={{
                                  ...
                                  >
                                    Khả dụng: {invItem.availableQuantity}{" "}
                                    {globalItem?.unit || invItem.unit || "gói"}
                                  </span>
                                </div>
                                {globalItem?.amountPerPackage && (
                                  <div
                                  ...
                                  >
                                    Quy cách: {globalItem.amountPerPackage}{" "}
                                    {globalItem.unit || invItem.unit} /{" "}
                                    {globalItem.packaging || "gói"}
                                  </div>
                                )}
                              </div>
                            </ItemCardBase>
        ...
        <div className="stp-col" style={{ gridColumn: "3" }}>
          <div
            className={`stp-card stp-card--full-height stp-confirm-card ${summaryData.length === 0 ? "stp-card--dim" : ""}`}
          >
            <div className="stp-card__header">
                  ...
                  {summaryData.map((d) => (
                    <div key={d.roomId} className="stp-summary__room-block">
                      ...
                      {d.items.map((i) => (
                        <div
                          key={i.itemId}
                          className={`stp-summary__chip stp-summary__chip--lg ${i.error ? "stp-summary__chip--err" : ""}`}
                        >
            ...
            <div className="stp-note-row">
              <StickyNote2Outlined
                style={{ fontSize: 15, color: "var(--stp-muted)" }}
              />
              <input
                className="stp-input"
                placeholder="Ghi chú..."
            ...
            <button
              className="stp-confirm-btn stp-confirm-btn--revoke"
              disabled={!canConfirm}
